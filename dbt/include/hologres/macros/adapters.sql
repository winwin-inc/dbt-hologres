{% macro hologres__create_table_as(temporary, relation, sql) -%}
...
{% macro hologres__get_create_index_sql(relation, index_dict) -%}
...
{% macro hologres__information_schema_name(database) -%}
  {% if database_name -%}
...
{% macro hologres__make_relation_with_suffix(base_relation, suffix, dstring) %}
    {% if dstring %}
...
{% macro hologres__make_intermediate_relation(base_relation, suffix) %}
    {{ return(hologres__make_relation_with_suffix(base_relation, suffix, dstring=False)) }}
{% endmacro %}

{% macro hologres__make_temp_relation(base_relation, suffix) %}
    {% set temp_relation = hologres__make_relation_with_suffix(base_relation, suffix, dstring=True) %}
    {{ return(temp_relation.incorporate(path={"schema": none,
                                              "database": none})) }}
{% endmacro %}

{% macro hologres__make_backup_relation(base_relation, backup_relation_type, suffix) %}
    {% set backup_relation = hologres__make_relation_with_suffix(base_relation, suffix, dstring=False) %}
...
#}
{% macro hologres_escape_comment(comment) -%}
...
{% macro hologres__alter_relation_comment(relation, comment) %}
  {% set escaped_comment = hologres_escape_comment(comment) %}
  comment on {{ relation.type }} {{ relation }} is {{ escaped_comment }};
{% endmacro %}


{% macro hologres__alter_column_comment(relation, column_dict) %}
...
{%- macro hologres__get_show_grant_sql(relation) -%}
...
{% macro hologres__copy_grants() %}
    {{ return(False) }}
{% endmacro %}


{% macro hologres__get_show_indexes_sql(relation) %}
    select
        i.relname                                   as name,
        m.amname                                    as method,
...
{%- macro hologres__get_drop_index_sql(relation, index_name) -%}
